...
function familyInfo(c) {
  let discountHint = "17 лет = взрослый на всех линиях; детских скидок нет";
  if (c.thirdGuestDiscount) {
    discountHint = "3-й гость дешевле — смотрите «€ 3 чел.»";
  } else if (c.price2 && c.price3 && c.pricePP) {
    const thirdPp = c.price3 / 3;
    if (thirdPp < c.pricePP * 0.95) {
      discountHint = `3-й гость ~€${Math.round(thirdPp)}/чел vs €${c.pricePP}/чел`;
    }
  }
  return {
    summary: "Дочка 17 лет — полный взрослый тариф",
    steps: [
      "Бронируйте 3 взрослых (или 2 + проверьте 3-й гость promo)",
      "Сравните «€ 2 чел.» и «€ 3 чел.»",
      "На сайте линии укажите 3 гостей",
    ],
    discountHint,
    policyUrl: c.lineUrl,
  };

#!/usr/bin/env node
/** Build med-summer-july-2026.html from research/summer-med-july-2026.json */
import fs from "fs";
...
const outPath = path.join(root, "med-summer-july-2026.html");
...
<title>Med Summer 2026 — круизы из Европы (июль–август)</title>
<style>
:root { --blue:#007AFF; --green:#34C759; --orange:#FF9500; --bg:#F2F2F7; --card:#fff; --text:#1C1C1E; --text2:#636366; --sep:#E5E5EA; }
...
.chip.fr.on { background:var(--green); border-color:var(--green); }
...
<body>
<h1>Круизы из Европы · июль–август 2026</h1>
<p class="sub">Отправление с 5 июля · ${meta.ports.length} портов · ${meta.source}, ${meta.fetchedAt}${meta.enrichedAt ? " · цены площадок " + meta.enrichedAt : ""}</p>
<div class="callout">
  <strong>Фильтры:</strong> порт, <strong>ночей</strong>, сортировка по дате/цене.
  <strong>€ 2 / € 3 чел.</strong> — итого за каюту (inside).
  <strong>Купить</strong> — раскройте список площадок (линия, Cruisello, агентства).
  Дочка <strong>16 лет</strong>: нажмите «Как проверить скидку» в колонке корабля.
</div>
...
function childTip(c) {
  ...
  return '<details class="child-tip fold"><summary>Как проверить скидку (16 лет)</summary>' +
    "<p><strong>" + (t.summary || "") + "</strong></p>" +
    "<p>" + (t.discountHint || "") + "</p>" +
    (steps ? "<ol>" + steps + "</ol>" : "") +
    (t.policyUrl ? '<p><a href="' + t.policyUrl + '" target="_blank" rel="noopener">Политика линии ↗</a></p>' : "") +
    "</details>";
}
...
function render() {
  ...
  document.getElementById("stats").innerHTML = \`
    ...
    <div class="stat"><b>\${fr.length}</b><span>из Франции</span></div>
    <div class="stat"><b>\${best ? fmtEur(best.price2) : "—"}</b><span>мин. 2 чел.</span></div>
    <div class="stat"><b>\${NIGHTS.length-1}</b><span>вариантов ночей</span></div>\`;

  document.getElementById("port-filters").innerHTML = PORTS.map(p =>
    \`<button class="chip\${p==="Marseille"||p==="Cannes"?" fr":""}\${portFilter===p?" on":""}" data-port="\${p}">\${p==="all"?"Все порты":p}</button>\`
  ).join("");
  ...
  document.getElementById("tbody").innerHTML = list.map(c => {
    const hl = c.port === "Cannes" && c.sailDate === "2026-07-05" ? ' class="highlight"' : "";
    const tags = [];
    if (c.port === "Cannes" && c.sailDate === "2026-07-05") tags.push('<span class="badge best">5 Jul · Франция</span>');
